...
Original file is located at
    https://colab.research.google.com/drive/1PyXvEyStTvMzZbcMWAakuk3GyvwGzUGU
"""
    ...
    class myCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs=None):
            if(logs.get('mae')<0.09 and logs.get('val_mae')<0.09):
                print("\nMAE training and validation have been smaller than 0.9")
                self.model.stop_training = True
    callback = myCallback()
    # Code to train and compile the model
    model.compile(loss='mae', metrics=['mae'], optimizer='adam')
    model.fit(train_set, epochs=10, validation_data=valid_set, verbose=1, callbacks=[myCallback()])
    # YOUR CODE HERE
    return model
